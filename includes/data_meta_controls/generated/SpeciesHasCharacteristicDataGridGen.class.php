<?php
	 ...
	 * listing a collection of SpeciesHasCharacteristic objects.  It includes
	 ...
	 * 
	 */
	class SpeciesHasCharacteristicDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SpeciesHasCharacteristic datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SpeciesHasCharacteristic object, or it can be any QQNode extending from QQN::SpeciesHasCharacteristic().
		 * 
		 ...
		 * sorting using two fields from the SpeciesHasCharacteristic object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SpeciesHasCharacteristic
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intSpeciesIdspecies=<?=urlencode($_ITEM->SpeciesIdspecies)?>&intCharacteristicIdcharacteristic=<?=urlencode($_ITEM->CharacteristicIdcharacteristic)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->SpeciesIdspecies))?>/<?=str_replace("+","%20",urlencode($_ITEM->CharacteristicIdcharacteristic))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->SpeciesIdspecies . "," . $_ITEM->CharacteristicIdcharacteristic, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = SpeciesHasCharacteristic::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from SpeciesHasCharacteristic, given the clauses above
			$this->DataSource = SpeciesHasCharacteristic::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SpeciesHasCharacteristic-based QQNode.
		 * It will also verify that it is a proper SpeciesHasCharacteristic-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'species_has_characteristic') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "species_has_characteristic".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'SpeciesIdspecies': return QQN::SpeciesHasCharacteristic()->SpeciesIdspecies;
				case 'SpeciesIdspeciesObject': return QQN::SpeciesHasCharacteristic()->SpeciesIdspeciesObject;
				case 'CharacteristicIdcharacteristic': return QQN::SpeciesHasCharacteristic()->CharacteristicIdcharacteristic;
				case 'CharacteristicIdcharacteristicObject': return QQN::SpeciesHasCharacteristic()->CharacteristicIdcharacteristicObject;
				default: throw new QCallerException('Simple Property not found in SpeciesHasCharacteristicDataGrid content: ' . $mixContent);
