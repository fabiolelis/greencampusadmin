<?php
	 ...
	 * listing a collection of Event objects.  It includes
	 ...
	 * 
	 */
	class EventDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Event datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Event object, or it can be any QQNode extending from QQN::Event().
		 * 
		 ...
		 * sorting using two fields from the Event object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Event
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdevent=<?=urlencode($_ITEM->Idevent)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->Idevent))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Idevent, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Event::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Event, given the clauses above
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Event-based QQNode.
		 * It will also verify that it is a proper Event-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'event') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "event".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Idevent': return QQN::Event()->Idevent;
				case 'DateTime': return QQN::Event()->DateTime;
				case 'Location': return QQN::Event()->Location;
				case 'Description': return QQN::Event()->Description;
				case 'Images': return QQN::Event()->Images;
				case 'Videos': return QQN::Event()->Videos;
				default: throw new QCallerException('Simple Property not found in EventDataGrid content: ' . $mixContent);
