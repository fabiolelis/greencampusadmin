<?php
	 ...
	 * listing a collection of Species objects.  It includes
	 ...
	 * 
	 */
	class SpeciesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Species datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Species object, or it can be any QQNode extending from QQN::Species().
		 * 
		 ...
		 * sorting using two fields from the Species object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Species
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdspecies=<?=urlencode($_ITEM->Idspecies)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->Idspecies))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Idspecies, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Species::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Species, given the clauses above
			$this->DataSource = Species::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Species-based QQNode.
		 * It will also verify that it is a proper Species-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'species') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "species".');
