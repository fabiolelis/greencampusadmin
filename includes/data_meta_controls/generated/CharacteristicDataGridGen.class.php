<?php
	 ...
	 * listing a collection of Characteristic objects.  It includes
	 ...
	 * 
	 */
	class CharacteristicDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Characteristic datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Characteristic object, or it can be any QQNode extending from QQN::Characteristic().
		 * 
		 ...
		 * sorting using two fields from the Characteristic object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Characteristic
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdcharacteristic=<?=urlencode($_ITEM->Idcharacteristic)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->Idcharacteristic))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Idcharacteristic, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Characteristic::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Characteristic, given the clauses above
			$this->DataSource = Characteristic::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Characteristic-based QQNode.
		 * It will also verify that it is a proper Characteristic-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'characteristic') {
